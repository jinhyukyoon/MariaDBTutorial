...
bool MariaDBConnector::connect(const char* host, const char* user, const char* passwd, int port)
{
    if (checkInitialized())
    {
        try
        {
            ...
            if (port < 0)
            {
                url = std::format("{}{}", "jdbc:mariadb://", std::string(host));

            }
            else
            {
                url = std::format("{}{}/{}", "jdbc:mariadb://", std::string(host), std::to_string(port));
            }
...
bool MariaDBConnector::query(const char* query_content)
{
    if (checkInitialized())
    {
        try
        {
            ...
            sql::SQLString query(std::format("{}", query_content));

            if (sql::ResultSet* res = stmt->executeQuery(query); res != nullptr)
            {
                if (res->next())
                {
